 ...
 * import is all-or-nothing across both tables, so an admin reading this list is
 * about to edit the spreadsheet and upload it again.
 */
function describe(error: ImportError): string {
  ...
  switch (error.code) {
    ...
    case "INVITE_WITHOUT_EMAIL":
      return `${at}${who}：要开通门户账号必须填邮箱 —— 激活时是靠邮箱认人的。请补上邮箱，或把「开通门户」改成「否」。`;
    case "NO_SEASON":
      return `${at}${who}：缺少季度。请在「季度」列填写至少一个季度，多个用分号隔开（例如「2026秋季;2027春季」）。`;
    case "UNKNOWN_SEASON":
      return `${at}${who}：季度「${error.value}」不存在。请在「季度管理」中先新建该季度，或改用已有季度的名称。`;
    case "UNKNOWN_GROUP":
      return `${at}${who}：组别「${error.value}」不存在。请在「志愿者名单」页下方的组别管理中先新建，或改用已有组别的名称。`;
    case "LEAD_WITHOUT_GROUP":
      return `${at}${who}：「${error.value}」标记了负责人却没有写组别。负责人是某个组的负责人，请写成「${error.value}:运营组(负责人)」的形式。`;
    case "GROUP_WITHOUT_VOLUNTEER":
      return `${at}${who}：填了组别「${error.value}」但身份里没有志愿者。组别只对志愿者有意义 —— 请把身份改成「志愿者」，或清空组别。`;
    case "FILE_DUP_EMAIL":
